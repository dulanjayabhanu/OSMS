...
$teacherResultset = Database::search("SELECT * FROM `teacher` WHERE `email`='" . $email . "' AND `user_type`='1'");
...
if ($teacherRownumber == 1) {
    ...
    if (empty($firstname)) {
        echo ("Please enter teacher's first name");
    } else if (empty($lastname)) {
        echo ("Please enter teacher's last name");
    } else if (strlen($firstname) > 50) {
        echo ("Teacher's first name should be <50 characters");
    } else if (strlen($lastname) > 50) {
        echo ("Teacher's last name should be <50 characters");
    } else if (empty($email)) {
        echo ("Please enter teacher's email address");
    } else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        echo ("Invalid teacher's email address");
    } else if (strlen($email) > 100) {
        echo ("Teacher's email should be <100 characters");
    } else if (empty($mobile)) {
        echo ("Please enter teacher's mobile");
    } else if (strlen($mobile) != 10) {
        echo ("Teacher's mobile must have 10 characters");
    } else if (!preg_match("/07[0,1,2,4,5,6,7,8][0-9]/", $mobile)) {
        echo ("Invalied mobile number");
    } else if ($gender == 0) {
        echo ("Please select teacher's gender");
    } else {
        ...
        VALUES ('" . $email . "','" . $password . "','" . $firstname . "','" . $lastname . "','" . $mobile . "','" . $date . "','1','2','" . $gender . "','".$userName."','1')");
        ...
        <p style="text-align: justify;">The purpose of this message is to inform you that you have been invited to contribute as a teacher to the online student management system. To accept this invitation, click the button below.</p><br/>
        ...
        <a href="http://localhost/osms/teacherSignIn.php?code=include" style="border-radius: 24px;padding-top: 8px;padding-bottom: 8px;padding-left: 15px;padding-right: 15px;background-color:#3972eb; color: #fff;font-family: "Arial";font-weight: bold;">Accept Invitation</a>
