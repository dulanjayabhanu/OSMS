...
if (empty($username)) {
    ...
    $officerResultset = Database::search("SELECT * FROM `teacher` WHERE `username`='" . $username . "' AND `password`='" . $password . "' AND `user_type`='2'");
    $officerRownumber = $officerResultset->num_rows;

    if ($officerRownumber == 1) {
        $officerData = $officerResultset->fetch_assoc();

        $uniqueCodeRsultset = Database::search("SELECT * FROM `unique_code` WHERE `teacher_email`='" . $officerData["email"] . "' AND `name`='" . $oneTimeCode . "'");
        $uniqueCodeRownumber = $uniqueCodeRsultset->num_rows;

        if ($uniqueCodeRownumber == 1) {
            $uniqueCodeData = $uniqueCodeRsultset->fetch_assoc();

            $startDate = new DateTime($officerData["joined_date"]);
            $tdate = new DateTime();
